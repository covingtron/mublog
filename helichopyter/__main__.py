...
class _HclGenerator:
    ...
    def process_definitions(self) -> None:
        append_specs: dict[str, tuple[Any, slice | int]] = {
            'provider': (self.providers, slice(1, 3)),
            'locals': (self.locals_blocks, 1),
            'resource': (self.resources, slice(1, 5)),
            'data': (self.data_sources, slice(1, 4)),
            'variable': (self.variables, slice(1, 3)),
            'output': (self.outputs, slice(1, 3)),
        }
        ...
        for definition in definitions:
            ...
            if type_ == 'terraform':
                ...
                elif subtype == 'backend':
                    self.terraform_config['backend'] = (definition[2], definition[3])
                continue

            if type_ not in append_specs:
                continue

            target, payload = append_specs[type_]
            if isinstance(payload, slice):
                target.append(tuple(definition[payload]))
            else:
                target.append(definition[payload])
    ...
    def _generate_terraform(self, output_lines: list[str]) -> None:
        ...
        if 'required_providers' in self.terraform_config:
            ...
            for prov_name, prov_config in self.terraform_config['required_providers'].items():
                ...
                output_lines.append('    }')
            output_lines.append('  }')
        if 'backend' in self.terraform_config:
            backend_name, backend_config = self.terraform_config['backend']
            output_lines.append(f'  backend "{backend_name}" {{')
            for k, v in backend_config.items():
                output_lines.append(f'    {k} = {format_value(v, 2)}')
            output_lines.append('  }')
        output_lines.append('}')
        output_lines.append('')
